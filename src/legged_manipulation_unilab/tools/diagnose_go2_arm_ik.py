...
def _state_qpos_qvel(backend: Any) -> tuple[np.ndarray, np.ndarray]:
    state = backend.get_state(fields=["qpos", "qvel"])
    qpos = np.asarray(state["qpos"][0], dtype=np.float64).copy()
    qvel = np.asarray(state["qvel"][0], dtype=np.float64).copy()
    return qpos, qvel
...
def _force_go2_home(env: Any, home_qpos: np.ndarray) -> None:
    ...
    first_arm_qpos = int(backend.get_joint_state_qpos_indices(env._cfg.asset.arm_joint_names)[0])
...
def run_jacobian_fd_check(env: Any, eps: float) -> dict[str, Any]:
    ...
    for col, qpos_idx in enumerate(
        backend.get_joint_state_qpos_indices(env._cfg.asset.arm_joint_names)
    ):
        qpos = qpos0.copy()
        qpos[qpos_idx] += eps
