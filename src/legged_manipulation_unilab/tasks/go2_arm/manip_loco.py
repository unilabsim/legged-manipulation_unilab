...
from legged_manipulation_unilab.tasks.common import rewards
from legged_manipulation_unilab.tasks.common.commands import Commands
from legged_manipulation_unilab.tasks.common.domain_rand import DomainRandConfig
from legged_manipulation_unilab.tasks.common.dr_provider import LocomotionDRProvider
...
class Go2ArmManipLocoEnv(Go2ArmBaseEnv):
    ...
    def __init__(self, cfg: Go2ArmManipLocoCfg, num_envs=1, backend_type="mujoco"):
        ...
        scene.model_file = resolve_scene(scene.model_file)
    ...
    def _sample_goal_spheres(self, env_ids: np.ndarray, start_spheres: np.ndarray) -> None:
        ...
        for _ in range(max(1, cfg.num_resample_attempts)):
            length = np.random.uniform(*cfg.sphere_l_range, size=len(remaining)).astype(dtype)
            ...
            new_goals = np.stack([length, phi, theta], axis=1)
